...
@section('content_header')
    <h1 class="mb-3">Buat Jurnal Manual</h1>
@endsection
...
@section('content')
    <div class="card shadow-sm border-0">
        <div class="card-body">
            ...
            <form action="{{ route('jurnal_umum.store') }}" method="POST" id="formJurnal" novalidate>
                @csrf
                ...
                <table class="table table-bordered table-sm" id="tableJurnal">
                    <thead class="thead-light">
                        <tr>
                            <th style="width: 35%;">Kode Akun <span class="text-danger">*</span></th>
                    ...
                    <tbody>
                        ...
                        @if (count($oldDetails) > 0)
                            @foreach ($oldDetails as $index => $detail)
                                <tr>
                                    <td>
                                        <select name="details[{{ $index }}][kode_akun]" class="form-control"
                                            required>
                        ...
                        @else
                            <tr>
                                <td>
                                    <select name="details[0][kode_akun]" class="form-control" required>
                                        <option value="" disabled selected>-- Pilih Akun --</option>
                                        @foreach ($coa as $akun)
                                            <option value="{{ $akun->kode_akun }}">
                                                {{ $akun->kode_akun }} - {{ $akun->nama_akun }}
                                ...
                                <td>
                                    <input type="number" step="0.01" min="0" name="details[0][debit]"
                                        class="form-control debit" value="0">
                                </td>
                                <td>
                                    <input type="number" step="0.01" min="0" name="details[0][kredit]"
                                        class="form-control kredit" value="0">
                                </td>
                                <td>
                                    <input type="text" name="details[0][keterangan]" class="form-control" value="">
                                </td>
                                <td class="text-center align-middle">
                    ...
                    <tfoot>
                        ...
                        <tr>
                            ...
                            <th><input type="text" id="totalDebit"
                                    class="form-control-plaintext text-right font-weight-bold" readonly value="0"></th>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#formJurnal').on('submit', function(e) {
                ...
                let totalKredit = parseFloat($('#totalKredit').val());
                ...
                if (totalDebit !== totalKredit) {
                    alert('Total Debit dan Kredit harus sama (seimbang).');
                    e.preventDefault();
                    return false;
                }
